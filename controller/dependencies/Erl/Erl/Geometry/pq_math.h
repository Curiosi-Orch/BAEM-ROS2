...
#ifndef ERL_PQ_MATH_H
#define ERL_PQ_MATH_H

#include <utility>
#include <algorithm>
#include "Erl/Core.h"
//#include "rm_poly4_root.h"
...
template<typename T> inline void null_n(const Vector3<T> & _in_w, Vector3<T>& _out_u, Vector3<T>& _out_v )
{
//    uint_fast8_t l0 = (std::fabs(_in_w[0]      ) < std::fabs(_in_w[1])) ? 1u :  0u   ;
//    uint_fast8_t l1 = (std::fabs(_in_w[(1u-l0)]) < std::fabs(_in_w[2])) ? 2u : (1u-l0);
//    uint_fast8_t  s = (~(l0^l1)) & 3u;

//    uint_fast8_t l0;
//    uint_fast8_t l1;
//    uint_fast8_t  s;

    if(std::fabs(_in_w[0] ) < std::fabs(_in_w[1]))
    {
        if(std::fabs(_in_w[0] )< std::fabs(_in_w[2]))
        { //s = 0; l0 =1; l1 =2;
          _out_u = Erl::Vector3<T>( -_in_w[2],(2*_in_w[2]*_in_w[0])/_in_w[1],-_in_w[0]).normalized();
        }
        else
        { //s = 2; l0 =0; l1 =1;
          _out_u = Erl::Vector3<T>((2*_in_w[1]*_in_w[2])/_in_w[0],-_in_w[2],-_in_w[1]).normalized();
        }
    }
    else
    {
        if(std::fabs(_in_w[1] )< std::fabs(_in_w[2]))
        { //s = 1; l0 =0; l1 =2;
          _out_u = Erl::Vector3<T>( (2*_in_w[2]*_in_w[1])/_in_w[0], -_in_w[2], -_in_w[1]).normalized();
        }
        else { //s = 2; l0 =1; l1 =0;
              _out_u = Erl::Vector3<T>(-_in_w[2],(2*_in_w[0]*_in_w[2])/_in_w[1],-_in_w[0]).normalized();
        }
    }
...
//    _out_u[s ] = -_in_w[l1];
//    _out_u[l1] = -_in_w[ s];
//    _out_u[l0] = (2*_in_w[l1]*_in_w[s])/_in_w[l0];



    _out_v = _in_w.cross(_out_u);


}

template<typename T> inline void null(const Vector3<T> & _in_w, Vector3<T>& _out_u, Vector3<T>& _out_v )
{
    uint_fast8_t l0 = (std::fabs(_in_w[0]      ) < std::fabs(_in_w[1])) ? 1u :  0u   ;
    uint_fast8_t l1 = (std::fabs(_in_w[(1u-l0)]) < std::fabs(_in_w[2])) ? 2u : (1u-l0);
    uint_fast8_t  s = (~(l0^l1)) & 3u;

    if(std::fabs(_in_w[l0]) < std::fabs(_in_w[l1]))
    {std::swap<uint_fast8_t>(l0,l1);}
//    _out_u[s ] =      T(0.);
//    _out_u[l0] = -_in_w[l1];
//    _out_u[l1] =  _in_w[l0];
    ...
    _out_u[s ] =  _in_w[l1];
    _out_u[l1] =  _in_w[l0];
    _out_u[l0] = -_in_w[l1] - (_in_w[l1]*_in_w[s])/_in_w[l0];
...
#if defined(USE_GSL) || defined(FORCE_USE_GSL)
#include <gsl/gsl_poly.h>
template<typename T> inline unsigned root_poly(const size_t _degree, const T *in_coeff, T *out_root)
{
    typedef double T2;
    T2 * c_in_coeff = reinterpret_cast<T2*>(alloca(1+_degree*sizeof(T2)));

    for(size_t iCoeff(0);iCoeff<_degree;iCoeff++)
    {
        c_in_coeff[_degree-1-iCoeff] = in_coeff[iCoeff];
    }
    c_in_coeff[_degree] = 1.0;


    gsl_poly_complex_workspace * c_gsl_ws
        = gsl_poly_complex_workspace_alloc(_degree+1);

    T2 * c_complex_roots = reinterpret_cast<T2*>(alloca(2*_degree*sizeof(T2)));

    gsl_poly_complex_solve (c_in_coeff, _degree+1, c_gsl_ws, c_complex_roots);
    size_t c_Roots(0);
    for(size_t iR(0);iR<_degree;iR++)
    {
        if(std::fabs(c_complex_roots[2*iR+1])<Erl::Constants<T>::Zero_Tolerance_Soft)
        {
            out_root[c_Roots] = c_complex_roots[2*iR];
            c_Roots++;
        }
    }
    return c_Roots;
...
#endif
//template<typename T> inline unsigned root_poly2(const T *in_coeff, T *out_root){
...
{
    #ifdef FORCE_USE_GSL
    return root_poly(2,in_coeff,out_root);
    #else
...
template<typename T> inline unsigned root_poly3(const T *in_coeff, T *out_root)
{
    #ifdef FORCE_USE_GSL
    return root_poly(3,in_coeff,out_root);
    #else
...
template<typename T> inline unsigned root_poly4(const T *in_coeff, T *out_root)
{
    #ifdef FORCE_USE_GSL
    return root_poly(4,in_coeff,out_root);
    #else
    /*
